...
human_face_num_in_human_files = 0
human_face_num_in_dog_files = 0
for i in range(100):
    if face_detector(human_files_short[i]):
        human_face_num_in_human_files += 1
    if face_detector(dog_files_short[i]):
        human_face_num_in_dog_files += 1

print("Detect human faces in human files probility:{}%".format(human_face_num_in_human_files))
print("Detect human faces in dog files probility:{}%".format(human_face_num_in_dog_files))
...
# 
# __回答:__
# 不合理，可以创建一个CNN，通过各种情况下的人脸图片来训练该神经网络，从而达到在图像中面部特征不清晰的情况下依然能够检测到人脸。
# 
...
dog_face_num_in_human_files = 0
dog_face_num_in_dog_files = 0
for i in range(len(human_files_short)):
    if dog_detector(human_files_short[i]):
        dog_face_num_in_human_files += 1
    if dog_detector(dog_files_short[i]):
        dog_face_num_in_dog_files += 1

print("Detect dog face in human files probility:{}%".format(dog_face_num_in_human_files))
print("Detect dog face in human files probility:{}%".format(dog_face_num_in_dog_files))
...
# 
# 
# 现在我们已经实现了一个函数，能够在图像中识别人类及狗狗。但我们需要更进一步的方法，来对狗的类别进行识别。在这一步中，你需要实现一个卷积神经网络来对狗的品种进行分类。你需要__从头实现__你的卷积神经网络（在这一阶段，你还不能使用迁移学习），并且你需要达到超过1%的测试集准确率。在本项目的步骤五中，你还有机会使用迁移学习来实现一个准确率大大提高的模型。
# 
...
bottleneck_features2 = np.load('/data/bottleneck_features/DogResnet50Data.npz')
...
bottleneck_features3 = np.load('/data/bottleneck_features/DogInceptionV3Data.npz')
train_Inception = bottleneck_features3['train']
valid_Inception = bottleneck_features3['valid']
test_Inception = bottleneck_features3['test']

bottleneck_features4 = np.load('/data/bottleneck_features/DogXceptionData.npz')
...
# 
# 最终四个模型的预测精度分别为46.4115%(VGG19_predictions)、80.9809%(ResNet50)、77.7512%(Inception)、84.9282%(Xception)。
# 
...
Resnet50_model.compile(loss='categorical_crossentropy', optimizer='Adagrad', metrics=['accuracy'])
...
### TODO: 训练模型
checkpointer1 = ModelCheckpoint(filepath='saved_models/weights.best.VGG19.hdf5', 
                               verbose=1, save_best_only=True)
...
checkpointer2 = ModelCheckpoint(filepath='saved_models/weights.best.ResNet50.hdf5', verbose=1, save_best_only=True)
Resnet50_model.fit(train_Resnet50, train_targets, 
          validation_data=(valid_Resnet50, valid_targets),
          epochs=20, batch_size=20, callbacks=[checkpointer2], verbose=1)


checkpointer3 = ModelCheckpoint(filepath='saved_models/weights.best.Inception.hdf5', 
                               verbose=1, save_best_only=True)
...
checkpointer4 = ModelCheckpoint(filepath='saved_models/weights.best.Xception.hdf5', 
                               verbose=1, save_best_only=True)
...
test_accuracy1 = 100*np.sum(np.array(VGG19_predictions)==np.argmax(test_targets, axis=1))/len(VGG19_predictions)
print('Test accuracy: %.4f%% -- (VGG19_predictions)' % test_accuracy1)
...
print('Test accuracy: %.4f%% -- (ResNet50)' % test_accuracy2)
...
print('Test accuracy: %.4f%% -- (Inception)' % test_accuracy3)

Xception_predictions = [np.argmax(Xception_model.predict(np.expand_dims(feature, axis=0))) for feature in test_Xception]
test_accuracy4 = 100*np.sum(np.array(Xception_predictions)==np.argmax(test_targets, axis=1))/len(Xception_predictions)
print('Test accuracy: %.4f%% -- (Xception)' % test_accuracy4)
...
predict_dog(train_files[0])


# In[41]:


predict_dog(human_files[0])
...
def predict_img(img_path):
    ...
    if has_dog or has_face:
        ...
        category = predict_dog(img_path)
        title = ""
        x_label = ""
        if has_dog:
            title = "Hello, dog !"
            x_label = ("You are belong to %s ~" % category.split('.')[-1])
        else:
            title = "Hello, human !"
            x_label = ("You look like a ....\n %s" % category.split('.')[-1])
            
        a.set_title(title)
        a.set_xlabel(x_label)
        return category
        
    if (not has_face) and (not has_dog):
        a.set_title("There is no dog or human in the image!")
        

predict_img(train_files[0])
...
# 2. 提出至少三点改进你的模型的想法。
#     a、适当调整epoch
#     b、更换模型尝试，如将Resnet50 换为 Xception
#     c、增加卷积层数
...
## 自由地使用所需的代码单元数吧
img_paths = ["images/1.jpeg", "images/2.jpg", "images/3.jpeg", "images/4.jpeg", "images/5.jpeg", "images/6.jpg"]
